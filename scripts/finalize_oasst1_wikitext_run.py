"""Wait for the Fable 5 + WikiText training process, then evaluate and export it.

This script targets only the isolated mixed-data run. It never overwrites the
existing WikiText-only export or Ollama model.
"""
...
RUN_NAME = "oasst1_wikitext_mixed"
OLLAMA_MODEL_NAME = "kitai-oasst1-wikitext"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Only accept checkpoint_final.pt written after this Unix timestamp",
    )
...
def main() -> None:
    ...
    run_log_dir = project_dir / "logs" / RUN_NAME
    run_log_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir = project_dir / "checkpoints" / RUN_NAME
    tokenizer = project_dir / "tokenizer" / "kitai_bpe_32k.json"
    config = project_dir / "configs" / "oasst1_wikitext_mixed.yaml"
    export_dir = project_dir / "exports" / "kitai_oasst1_wikitext_hf"
    gguf_path = project_dir / "exports" / "kitai_oasst1_wikitext_f16.gguf"
    modelfile_path = project_dir / "exports" / "Modelfile.kitai_oasst1_wikitext"
    ...
    append_status(status_path, "Training process exited; selecting newest checkpoint from this run")
    candidates = [
        path for path in checkpoint_dir.glob("checkpoint*.pt")
        if path.stat().st_mtime >= args.not_before_epoch
    ]
    if not candidates:
        raise RuntimeError("Training exited without a new checkpoint; export was not attempted")
    selected_checkpoint = max(candidates, key=lambda path: path.stat().st_mtime)

    append_status(status_path, f"New checkpoint confirmed: {selected_checkpoint.name}; beginning evaluation and export")
    ...
    try:
        ...
        result["steps"]["evaluation"] = {
            "returncode": run_command(
                [
                    python, "-m", "scripts.evaluate", "--model", str(checkpoint),
                    "--tokenizer", str(tokenizer),
                    "--data", str(project_dir / "demo_data" / "wikitext103" / "val.txt"),
                    "--config", str(config), "--batch-size", "4", "--max-batches", "200", "--perplexity",
                ],
                evaluation_log,
                project_dir,
            ),
        ...
        result["steps"]["hf_export"] = {
            "returncode": run_command(
                [
                    python, str(project_dir / "scripts" / "export_checkpoint_to_hf.py"),
                    "--checkpoint", str(checkpoint), "--tokenizer", str(tokenizer),
                    "--output-dir", str(export_dir), "--overwrite",
                ],
                hf_export_log,
                project_dir,
            ),
        ...
        modelfile_path.write_text(
            ...
            "SYSTEM You are KitAI, a concise, helpful local language model.\n",
            encoding="utf-8",
        )
        ollama_import_log = run_log_dir / "post_training_ollama_import.log"
        result["steps"]["ollama_import"] = {
            "returncode": run_command(
                [str(ollama), "create", OLLAMA_MODEL_NAME, "-f", str(modelfile_path)],
                ollama_import_log,
                project_dir,
            ),
        ...
        result["steps"]["ollama_inference"] = {
            "returncode": run_command(
                [str(ollama), "run", OLLAMA_MODEL_NAME, "The capital of France is"],
                inference_log,
                project_dir,
            ),
