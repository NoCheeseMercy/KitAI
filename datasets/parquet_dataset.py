...
Loads Parquet files for training. Requires pyarrow or pandas.
"""
...
class ParquetDataset(BaseDataset):
    ...
    def __init__(
        ...
        if isinstance(file_paths, (str, Path)):
            ...
            if path.is_dir():
                self.file_paths = sorted(path.glob("*.parquet"))
            else:
        ...
        if not self.file_paths:
            raise ValueError(f"No .parquet files found: {file_paths}")

        self.load_data()

    def load_data(self) -> None:
        """Read Parquet files and create training samples."""
        try:
            import pandas as pd
        except ImportError:
